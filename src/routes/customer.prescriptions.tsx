...
export const Route = createFileRoute("/customer/prescriptions")({ component: CustomerPrescriptions });
    ...
    <div className="max-w-6xl mx-auto p-6 md:p-10">
      ...
      {(!prescriptions || prescriptions.length === 0) ? (
        <div className="rounded-2xl bg-card border p-12 shadow-card text-center">
          ...
          <p className="mt-2 text-muted-foreground">Your prescriptions will appear here once your pharmacist creates them.</p>
        </div>
      ) : (
        <Accordion type="single" collapsible className="space-y-3">
          {prescriptions.map((rx: any) => (
            <AccordionItem key={rx.id} value={rx.id} className="rounded-xl bg-card border shadow-card overflow-hidden">
              <AccordionTrigger className="px-6 py-4 hover:no-underline">
                <div className="flex items-center gap-4 text-left">
                  <div>
                    <div className="font-mono text-sm text-muted-foreground">{rx.prescription_number}</div>
              ...
              <AccordionContent className="px-6 pb-4">
                <div className="space-y-2">
                  {(rx.prescription_items ?? []).map((item: any) => (
                    <div key={item.id} className="flex items-center justify-between rounded-lg bg-muted/30 p-3 text-sm">
                      <div>
                        <div className="font-medium">{item.products?.name} {item.products?.strength}</div>
                        <div className="text-muted-foreground text-xs">{item.dosage} · {item.frequency} · {item.duration}</div>
                      </div>
                      <div className="text-mono text-sm">x{item.quantity_prescribed}</div>
                    </div>
                  ))}
                  {rx.notes && <div className="text-sm text-muted-foreground mt-2">Notes: {rx.notes}</div>}
                </div>
              </AccordionContent>
            </AccordionItem>
          ))}
