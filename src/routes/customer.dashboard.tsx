...
function tierLabel(points: number): { name: string; next: string; hint: string } {
  ...
  if (points >= 1000) return { name: "Gold", next: "Platinum", hint: `${2000 - points} pts to Platinum` };
...
function CustomerDash() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { points, activeRx: activeRx ?? 0, recentOrders: recentOrders ?? 0, tier: tierLabel(points) };
    },
    enabled: !!user,
  });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 md:p-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        ...
        <StatCard label="Active Prescriptions" value={String(stats?.activeRx ?? 0)} icon={FileText} tone="violet" />
        <StatCard label="Recent Orders" value={String(stats?.recentOrders ?? 0)} icon={ShoppingBag} tone="success" />
