...
function Prescriptions() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("prescriptions")
        .select("*, patients(full_name, phone, allergies)")
  ...
  const { data: rxItems } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        ...
        .select("*, products(name, strength, dosage_form, requires_prescription)")
        .eq("prescription_id", selectedRx.id);
      return data ?? [];
    },
  ...
  return (
    <PageShell>
      <PageHeader
        title="Prescriptions"
        subtitle="Dispensing queue across all branches"
        action={
          <div className="flex gap-2">
            <button
              onClick={() => setDosageCalc(true)}
              className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium hover:bg-muted"
            >
              <Calculator className="h-4 w-4" /> Dosage Calc
            </button>
            <button
              onClick={() => setCompoundCalc(true)}
              className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium hover:bg-muted"
            >
              <FlaskConical className="h-4 w-4" /> Compound
            </button>
            <button
              onClick={() => setDialogOpen(true)}
              className="inline-flex items-center gap-2 rounded-lg gradient-primary text-white font-semibold px-4 py-2.5 text-sm shadow-glow"
            >
      ...
      {!data?.length ? (
        <EmptyState
          ...
          message="Prescriptions you process will appear in the kanban board."
        />
      ) : (
            ...
            <div key={status} className="rounded-xl bg-muted/40 p-3 min-h-[400px]">
              <div className="flex items-center gap-2 mb-3">
                ...
                <Badge variant="outline" className="text-xs">
                  {(data ?? []).filter((p: any) => p.status === status).length}
                </Badge>
              </div>
                ...
                {(data ?? [])
                  ...
                  .map((p: any) => (
                    <div
                      ...
                      className="rounded-lg bg-card border p-3 shadow-card cursor-pointer hover:shadow-glow transition"
                    >
              ...
              <div className="mt-6 space-y-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-medium">{selectedRx.patients?.full_name ?? "—"}</div>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Phone:</span>
                    <div className="font-medium">{selectedRx.patients?.phone ?? "—"}</div>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Prescriber:</span>
                    <div className="font-medium">Dr. {selectedRx.prescriber_name}</div>
                  </div>
                  <div>
                    <span className="text-muted-foreground">License:</span>
                    <div className="font-medium">{selectedRx.prescriber_license ?? "—"}</div>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Issue Date:</span>
                    <div className="font-medium">{fmtDate(selectedRx.issue_date)}</div>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Expiry:</span>
                    <div className="font-medium">{fmtDate(selectedRx.expiry_date)}</div>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  {NEXT_STATUS[selectedRx.status] && (
                    <Button
                      onClick={() => advanceStatus.mutate(selectedRx)}
