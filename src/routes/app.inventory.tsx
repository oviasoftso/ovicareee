...
function Inventory() {
  const { data: products } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("products").select("id, sku, name, dosage_form, strength, reorder_level, selling_price, requires_prescription, is_controlled, controlled_schedule, barcode, categories(name)").order("name").limit(200);
      return data ?? [];
    },
  });
  ...
  const { data: expiring } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("inventory")
        .select("id, batch_number, expiry_date, quantity_in_stock, products(name, sku), branches(name)")
  ...
  const { data: lowStockItems } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("inventory")
        .select("product_id, quantity_in_stock, products(name, sku, reorder_level, reorder_quantity, supplier_id, cost_price, suppliers(name))")
      ...
      return Object.values(grouped).filter((g: any) => g.total <= (g.products?.reorder_level ?? 10));
    },
  });

  const lowCount = (products ?? []).filter((p: any) => (stockMap?.get(p.id) ?? 0) <= p.reorder_level).length;
  const expiringCount = expiring?.length ?? 0;
  const controlledCount = (products ?? []).filter((p: any) => p.is_controlled || p.controlled_schedule).length;
  ...
  return (
    <PageShell>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-5 mb-6">
        <StatCard label="Total Products" value={fmtNumber(products?.length ?? 0)} icon={Package} tone="primary" />
        <StatCard label="Low Stock" value={fmtNumber(lowCount)} hint="Below reorder level" icon={AlertTriangle} tone="amber" />
        <StatCard label="Expiring Soon" value={fmtNumber(expiringCount)} hint="Within 90 days" icon={Clock} tone="danger" />
        <StatCard label="Controlled" value={fmtNumber(controlledCount)} hint="Schedule I-V" icon={ShieldAlert} tone="violet" />
      </div>
                  ...
                  return (
                    <tr key={p.id} className="border-t hover:bg-muted/30">
                      ...
                      <td className="px-6 py-3 font-medium">
                        {p.name}
                        {p.requires_prescription && <Badge variant="outline" className="ml-1 text-[10px]">Rx</Badge>}
                        {p.controlled_schedule && <Badge variant="outline" className="ml-1 text-[10px] bg-violet/10 text-violet">C-{p.controlled_schedule}</Badge>}
                      </td>
        ...
        <TabsContent value="expiring">
          <div className="rounded-2xl bg-card border shadow-card overflow-hidden">
            {(!expiring || expiring.length === 0) ? (
              <div className="p-8 text-center text-muted-foreground">No items expiring within 90 days.</div>
            ) : (
              <table className="w-full text-sm">
                <thead className="bg-muted/50 text-muted-foreground">
                  <tr>
                ...
                <tbody>
                  {expiring.map((item: any) => {
                    ...
                    const urgency = daysLeft <= 30 ? "critical" : daysLeft <= 60 ? "warning" : "normal";
                    return (
                      <tr key={item.id} className="border-t hover:bg-muted/30">
                        ...
                        <td className="px-6 py-3 text-muted-foreground">{item.branches?.name ?? "—"}</td>
                        <td className="px-6 py-3 text-right text-mono">{item.quantity_in_stock}</td>
                        <td className="px-6 py-3 text-right text-mono text-xs">{fmtDate(item.expiry_date)}</td>
                        <td className="px-6 py-3 text-center">
                          <Badge className={urgency === "critical" ? "bg-red-100 text-red-800" : urgency === "warning" ? "bg-amber-100 text-amber-800" : "bg-green-100 text-green-800"}>
                            {daysLeft}d
                          </Badge>
                        </td>
                      </tr>
        ...
        <TabsContent value="controlled">
          <div className="rounded-2xl bg-card border shadow-card overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {(products ?? []).filter((p: any) => p.is_controlled || p.controlled_schedule).map((p: any) => (
                  <tr key={p.id} className="border-t hover:bg-muted/30">
                    <td className="px-6 py-3 text-mono text-xs">{p.sku}</td>
                    <td className="px-6 py-3 font-medium">{p.name} {p.strength}</td>
                    <td className="px-6 py-3 text-center">
                      <Badge className="bg-violet/10 text-violet font-mono">{p.controlled_schedule ?? "Yes"}</Badge>
                    </td>
                    <td className="px-6 py-3 text-muted-foreground">{p.categories?.name ?? "—"}</td>
                    <td className="px-6 py-3 text-right text-mono font-semibold">{fmtNumber(stockMap?.get(p.id) ?? 0)}</td>
                    <td className="px-6 py-3 text-right text-mono font-semibold text-primary">{fmtMoney(p.selling_price)}</td>
                  </tr>
                ))}
                {(products ?? []).filter((p: any) => p.is_controlled || p.controlled_schedule).length === 0 && (
                  <tr><td colSpan={6} className="px-6 py-8 text-center text-muted-foreground">No controlled substances in inventory.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <TabsContent value="reorder">
          <div className="rounded-2xl bg-card border shadow-card overflow-hidden">
            {(!lowStockItems || lowStockItems.length === 0) ? (
              <div className="p-8 text-center text-muted-foreground">All stock levels are above reorder thresholds.</div>
            ) : (
              <table className="w-full text-sm">
                <thead className="bg-muted/50 text-muted-foreground">
                  <tr>
                  ...
                  {lowStockItems.map((item: any) => {
                    ...
                    return (
                      <tr key={item.product_id} className="border-t hover:bg-muted/30">
                        ...
                        <td className="px-6 py-3 text-muted-foreground">{product?.suppliers?.name ?? "—"}</td>
                        <td className="px-6 py-3 text-right text-mono text-danger font-semibold">{item.total}</td>
                        <td className="px-6 py-3 text-right text-mono">{product?.reorder_level ?? 0}</td>
                        <td className="px-6 py-3 text-right text-mono font-semibold">{suggestedQty}</td>
                        <td className="px-6 py-3 text-right text-mono text-primary">{fmtMoney(estCost)}</td>
                      </tr>
                    );
                  })}
                </tbody>
