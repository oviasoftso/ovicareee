...
function CashDrawer() {
  ...
  const { data: sessions } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        ...
        .select(
          "*, opened_by_user:profiles!opened_by(full_name), closed_by_user:profiles!closed_by(full_name)",
        )
  ...
  return (
    <PageShell>
      <PageHeader
        title="Cash Drawer"
        subtitle="Track cash flow per shift"
        action={
          openSession ? (
            ...
            <Button onClick={() => setOpenDialog(true)} className="gradient-primary text-white">
              Open Drawer
            </Button>
          )
        }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-5 mb-6">
        <StatCard
          ...
          icon={Wallet}
          tone={openSession ? "success" : "amber"}
        />
        <StatCard
          label="Opening Amount"
          value={fmtMoney(openSession?.opening_amount ?? 0)}
          icon={DollarSign}
          tone="primary"
        />
        <StatCard
          label="Cash Sales"
          value={fmtMoney(todaySales?.total ?? 0)}
          hint={`${todaySales?.count ?? 0} transactions`}
          icon={TrendingUp}
          tone="violet"
        />
        <StatCard
          label="Expected"
          value={fmtMoney(Number(openSession?.opening_amount ?? 0) + (todaySales?.total ?? 0))}
          icon={CheckCircle}
          tone="success"
        />
      </div>
      ...
      <div className="rounded-2xl bg-card border shadow-card overflow-hidden">
        <div className="px-6 py-4 border-b">
          <h3 className="font-display font-semibold">Session History</h3>
        </div>
            ...
            {(sessions ?? []).map((s: any) => (
              <tr key={s.id} className="border-t hover:bg-muted/30">
                ...
                <td className="px-6 py-3 text-right text-mono">
                  {s.closing_amount ? fmtMoney(s.closing_amount) : "—"}
                </td>
                <td className="px-6 py-3 text-right text-mono">
                  {s.expected_amount ? fmtMoney(s.expected_amount) : "—"}
                </td>
                <td
                  className={`px-6 py-3 text-right text-mono font-semibold ${s.variance && s.variance !== 0 ? (s.variance > 0 ? "text-green-600" : "text-red-600") : ""}`}
                >
      ...
      <Dialog open={openDialog} onOpenChange={setOpenDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleOpen} className="gradient-primary text-white">
              Open Drawer
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          <div className="space-y-4">
            <div className="rounded-lg bg-muted/30 p-4 text-sm space-y-2">
              <div className="flex justify-between">
                <span>Opening:</span>
                <span className="text-mono">{fmtMoney(openSession?.opening_amount ?? 0)}</span>
              </div>
              <div className="flex justify-between">
                <span>Cash Sales:</span>
                <span className="text-mono">{fmtMoney(todaySales?.total ?? 0)}</span>
              </div>
              <div className="flex justify-between font-bold border-t pt-2">
                <span>Expected:</span>
                <span className="text-mono">
                  {fmtMoney(Number(openSession?.opening_amount ?? 0) + (todaySales?.total ?? 0))}
                </span>
              </div>
            </div>
            <div>
            ...
            {closingAmount && (
              <div
                className={`rounded-lg p-3 text-sm font-medium ${Number(closingAmount) - (Number(openSession?.opening_amount ?? 0) + (todaySales?.total ?? 0)) === 0 ? "bg-green-50 text-green-800" : "bg-amber-50 text-amber-800"}`}
              >
                Variance:{" "}
                {fmtMoney(
                  Number(closingAmount) -
                    (Number(openSession?.opening_amount ?? 0) + (todaySales?.total ?? 0)),
                )}
              </div>
            )}
          </div>
