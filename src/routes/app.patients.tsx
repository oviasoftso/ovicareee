  ...
  const { data: patientDetail } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: prescriptions }, { data: sales }, { data: loyalty }] = await Promise.all([
        supabase
          .from("prescriptions")
          .select("*, prescription_items(*, products(name, strength))")
          .eq("patient_id", selected.id)
          .order("created_at", { ascending: false })
          .limit(20),
        supabase
          .from("sales")
          .select("*, sale_items(product_name, quantity, total_price)")
          .eq("patient_id", selected.id)
            ...
            {(data ?? []).map((p: any) => (
              ...
              >
                ...
                <td className="px-6 py-3 text-right text-mono font-semibold text-primary">
                  {fmtNumber(p.loyalty_points)} pts
                </td>
              </tr>
            ))}
          </tbody>
              ...
              <div className="mt-6 space-y-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{fmtDate(selected.date_of_birth)}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="font-medium">{selected.blood_type ?? "—"}</div>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Loyalty:</span>
                    <div className="font-medium text-primary">
                      {selected.loyalty_points ?? 0} pts
                    </div>
                  </div>
                </div>
                ...
                {selected.insurance_provider && (
                  <div className="rounded-lg bg-muted/30 p-3 text-sm">
                    ...
                    {selected.insurance_provider} · {selected.insurance_number ?? "—"}
                  </div>
                )}
                ...
                <Tabs defaultValue="rx" className="mt-4">
                  <TabsList className="w-full">
                    <TabsTrigger value="rx" className="flex-1">
                      <FileText className="h-3 w-3 mr-1" /> Prescriptions
                    </TabsTrigger>
                  ...
                  <TabsContent value="loyalty" className="space-y-2 mt-3">
                    {(patientDetail?.loyalty ?? []).length === 0 && (
                      <div className="text-sm text-muted-foreground text-center py-4">
                        No transactions
                      </div>
                    )}
                    {(patientDetail?.loyalty ?? []).map((tx: any) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          {tx.points_earned > 0 && (
                            <div className="text-green-600 text-mono">+{tx.points_earned}</div>
                          )}
                          {tx.points_redeemed > 0 && (
                            <div className="text-red-600 text-mono">-{tx.points_redeemed}</div>
                          )}
                          <div className="text-xs text-muted-foreground">
