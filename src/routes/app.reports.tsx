  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: sales }, { data: items }, { data: products }] = await Promise.all([
        supabase
          .from("sales")
          .select("id, total_amount, sale_date, payment_method, branch_id")
      ...
      (sales ?? []).forEach((s: any) => {
        byMethod.set(
          s.payment_method,
          (byMethod.get(s.payment_method) ?? 0) + Number(s.total_amount),
        );
      });
      ...
      return {
        ...
        topProducts: Array.from(byProduct, ([name, v]) => ({ name, ...v }))
          ...
          .slice(0, 10),
        revenueByDay: Array.from(dailyMap, ([date, value]) => ({
          date: date.slice(5),
          value,
        })).slice(-30),
        forecastData,
  ...
  return (
    <PageShell>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          label="Transactions"
          value={fmtNumber(data.totalTx)}
          icon={Receipt}
          tone="primary"
        />
        <StatCard
          label="Units Sold"
          value={fmtNumber(data.totalUnits)}
          icon={Package}
          tone="violet"
        />
      ...
      <div className="rounded-2xl bg-card border p-6 shadow-card mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-display font-semibold flex items-center gap-2">
            <Activity className="h-5 w-5" /> Sales Forecast
          </h3>
          <div className="text-sm text-muted-foreground">
            Projected 30-day:{" "}
            <span className="font-semibold text-primary">{fmtMoney(data.forecast30)}</span>
          </div>
        </div>
