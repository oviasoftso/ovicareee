...
function Financials() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: sales }, { data: expenses }, { data: creditSales }] = await Promise.all([
        ...
        supabase.from("sales").select("id, receipt_number, total_amount, amount_paid, due_date, sale_date, patients(full_name)").eq("is_credit", true).order("sale_date", { ascending: false }),
      ]);
      ...
      const ar = (creditSales ?? []).filter((s: any) => Number(s.amount_paid) < Number(s.total_amount));
      const arTotal = ar.reduce((a: number, s: any) => a + (Number(s.total_amount) - Number(s.amount_paid)), 0);
      ...
      return { rev, exp, profit: rev - exp, margin: rev ? ((rev - exp) / rev) * 100 : 0, chart, branchData, expenseCategories, ar, arTotal, arAging };
    },
  });
  ...
  return (
    <PageShell>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        <div className="rounded-2xl bg-card border p-6 shadow-card">
          <h3 className="font-display font-semibold text-lg mb-4 flex items-center gap-2"><CreditCard className="h-5 w-5" /> Accounts Receivable</h3>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="rounded-lg bg-muted/30 p-3 text-center">
              <div className="text-sm text-muted-foreground">Total Outstanding</div>
              <div className="text-2xl font-display font-bold text-primary">{fmtMoney(data?.arTotal ?? 0)}</div>
            </div>
            <div className="rounded-lg bg-muted/30 p-3 text-center">
          ...
          <div className="grid grid-cols-4 gap-2 text-center text-sm">
            <div className="rounded-lg bg-green-50 p-2"><div className="text-xs text-muted-foreground">Current</div><div className="font-semibold text-green-700">{fmtMoney(data?.arAging?.current ?? 0)}</div></div>
            <div className="rounded-lg bg-amber-50 p-2"><div className="text-xs text-muted-foreground">30 days</div><div className="font-semibold text-amber-700">{fmtMoney(data?.arAging?.days30 ?? 0)}</div></div>
            <div className="rounded-lg bg-orange-50 p-2"><div className="text-xs text-muted-foreground">60 days</div><div className="font-semibold text-orange-700">{fmtMoney(data?.arAging?.days60 ?? 0)}</div></div>
            <div className="rounded-lg bg-red-50 p-2"><div className="text-xs text-muted-foreground">90+ days</div><div className="font-semibold text-red-700">{fmtMoney(data?.arAging?.days90 ?? 0)}</div></div>
          </div>
        </div>
      ...
      {data?.ar && data.ar.length > 0 && (
        <div className="rounded-2xl bg-card border shadow-card overflow-hidden">
          <div className="px-6 py-4 border-b"><h3 className="font-display font-semibold">Outstanding Invoices</h3></div>
              ...
              {data.ar.map((s: any) => {
                ...
                return (
                  <tr key={s.id} className="border-t hover:bg-muted/30">
                    ...
                    <td className="px-6 py-3 text-right text-mono font-semibold text-primary">{fmtMoney(outstanding)}</td>
                    <td className="px-6 py-3 text-xs text-muted-foreground">{s.due_date ? fmtDateTime(s.due_date) : "—"}</td>
                    <td className="px-6 py-3 text-center">
                      <Badge className={age > 90 ? "bg-red-100 text-red-800" : age > 60 ? "bg-orange-100 text-orange-800" : age > 30 ? "bg-amber-100 text-amber-800" : "bg-green-100 text-green-800"}>
                        {age}d
                      </Badge>
                    </td>
                  </tr>
