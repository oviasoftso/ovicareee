...
const features = [
  {
    ...
    desc: "Real-time stock across every branch, with batch & expiry tracking.",
  },
  {
    icon: Activity,
    title: "Lightning POS",
    desc: "Barcode-ready terminal, split payments, instant receipts.",
  },
  {
    icon: FileText,
    title: "Prescription Workflow",
    desc: "Kanban dispensing with allergy & interaction checks.",
  },
  {
    icon: BarChart3,
    title: "Financial Intelligence",
    desc: "P&L, tax, cash flow — drill from network to single sale.",
  },
  {
    icon: Building2,
    title: "Branch Network",
    desc: "Compare performance and rebalance stock across locations.",
  },
  {
    icon: Sparkles,
    title: "Loyalty Built-in",
    desc: "Tier progression, points redemption at checkout.",
  },
];
    ...
    <div className="min-h-screen bg-background">
      <header className="absolute top-0 left-0 right-0 z-20 px-6 lg:px-12 py-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            to="/register"
            className="rounded-full bg-white text-secondary px-5 py-2 text-sm font-semibold shadow-glow hover:scale-105 transition-transform"
          >
            Get started
          </Link>
        </div>
      ...
      <section className="relative gradient-hero min-h-[88vh] flex items-center overflow-hidden">
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(circle at 20% 30%, oklch(0.76 0.14 180 / 0.3), transparent 40%), radial-gradient(circle at 80% 70%, oklch(0.55 0.22 295 / 0.2), transparent 40%)",
          }}
        />
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center text-white">
          <div className="inline-flex items-center gap-2 rounded-full bg-white/10 backdrop-blur border border-white/20 px-4 py-1.5 mb-8 text-sm">
            <ShieldCheck className="h-4 w-4 text-primary-glow" /> A product of Ovia Software
            Solutions
          </div>
          ...
          <p className="mt-8 text-lg md:text-xl text-white/70 max-w-2xl mx-auto">
            OviCare unifies your branches, inventory, prescriptions, POS and financials in one calm,
            fast, beautifully designed system.
          </p>
          <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
            <Link
              to="/login"
              className="group inline-flex items-center gap-2 rounded-full bg-white text-secondary px-7 py-3.5 font-semibold shadow-glow hover:scale-105 transition-transform"
            >
              Staff Login{" "}
      ...
      <section className="py-20 px-6 bg-secondary text-white">
        <div className="max-w-5xl mx-auto grid md:grid-cols-3 gap-12 text-center">
          {[
            ...
            <div key={l}>
              <div className="text-5xl md:text-6xl font-display font-bold bg-gradient-to-r from-primary-glow to-white bg-clip-text text-transparent">
                {n}
              </div>
