...
import { Search, Plus, Minus, Trash2, CreditCard, Banknote, Smartphone, FileCheck, ShoppingCart, ScanBarcode, User, Award } from "lucide-react";
...
function POS() {
  ...
  const { data: products } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("products").select("id, name, selling_price, requires_prescription, sku, barcode, tax_rate").eq("is_active", true).limit(100);
      return data ?? [];
    },
  });
  ...
  const { data: patients } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from("patients").select("id, full_name, patient_code, loyalty_points, phone").ilike("full_name", `%${patientSearch}%`).limit(10);
      return data ?? [];
    },
  ...
  const filtered = useMemo(() => {
    ...
    return (products ?? []).filter((p: any) => p.name.toLowerCase().includes(q) || p.sku.toLowerCase().includes(q) || (p.barcode && p.barcode.includes(q)));
  }, [products, search]);
  ...
  const add = (p: any) => {
    setCart((c) => {
      ...
      return [...c, { id: p.id, name: p.name, price: Number(p.selling_price), qty: 1, taxRate: Number(p.tax_rate ?? 0.05) }];
    });
  };
  ...
  const handleBarcodeScan = async () => {
    ...
    if (product) {
      ...
      const { data } = await supabase.from("products").select("id, name, selling_price, requires_prescription, sku, barcode, tax_rate").eq("barcode", barcodeInput.trim()).eq("is_active", true).single();
      if (data) {
        add(data);
        setBarcodeInput("");
        barcodeRef.current?.focus();
  ...
  const complete = async () => {
    ...
    const { data: sale, error } = await supabase.from("sales").insert({
      ...
      subtotal, discount_amount: discountAmount, tax_amount: tax, total_amount: total,
    ...
    if (selectedPatient) {
      ...
      if (pointsEarned > 0) {
        ...
        await supabase.from("patients").update({ loyalty_points: (selectedPatient.loyalty_points ?? 0) + pointsEarned }).eq("id", selectedPatient.id);
      }
    }
        ...
        <div className="lg:col-span-3 rounded-2xl bg-card border shadow-card flex flex-col min-h-[600px]">
          <div className="p-5 border-b">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-display font-semibold flex items-center gap-2"><ShoppingCart className="h-4 w-4" /> Cart ({cart.length})</h3>
              <div className="relative">
                <button onClick={() => setShowPatientSearch(!showPatientSearch)} className="flex items-center gap-2 text-sm rounded-lg border px-3 py-1.5 hover:bg-muted transition">
                  <User className="h-4 w-4" /> {selectedPatient ? selectedPatient.full_name : "Link patient"}
                </button>
                {showPatientSearch && (
                  <div className="absolute right-0 top-full mt-1 w-72 bg-card border rounded-lg shadow-lg z-50 p-2">
                    <input value={patientSearch} onChange={(e) => setPatientSearch(e.target.value)} placeholder="Search patient..." className="w-full rounded border px-3 py-2 text-sm mb-2" autoFocus />
                    {(patients ?? []).map((p: any) => (
                      <button key={p.id} onClick={() => { setSelectedPatient(p); setShowPatientSearch(false); setPatientSearch(""); }} className="w-full text-left rounded p-2 hover:bg-muted text-sm">
                        <div className="font-medium">{p.full_name}</div>
                        <div className="text-xs text-muted-foreground">{p.patient_code} · {p.loyalty_points ?? 0} pts</div>
                      </button>
                    ))}
                  </div>
                )}
            ...
            {selectedPatient && (
              <div className="flex items-center gap-2 text-sm bg-primary/5 rounded-lg px-3 py-2">
                ...
                <span>{selectedPatient.full_name} · {selectedPatient.loyalty_points ?? 0} loyalty points</span>
                <button onClick={() => setSelectedPatient(null)} className="ml-auto text-muted-foreground hover:text-foreground">x</button>
              </div>
            )}
          </div>
          ...
          <div className="p-5 border-t bg-muted/30 space-y-3">
            ...
            <div className="flex justify-between text-sm items-center gap-2"><span>Discount</span>
              <div className="flex gap-1">
                <button onClick={() => setDiscountType(discountType === "flat" ? "percent" : "flat")} className="text-xs rounded border px-2 py-1 hover:bg-muted">{discountType === "flat" ? "$" : "%"}</button>
            ...
            <div className="flex justify-between text-sm text-muted-foreground"><span>Tax</span><span className="text-mono">{fmtMoney(tax)}</span></div>
        ...
        <div className="lg:col-span-2 rounded-2xl bg-card border shadow-card flex flex-col min-h-[600px]">
          <div className="p-5 border-b space-y-2">
            <div className="relative">
              ...
              <input ref={barcodeRef} value={barcodeInput} onChange={(e) => setBarcodeInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleBarcodeScan()} placeholder="Scan barcode..." className="w-full rounded-lg border bg-background pl-10 pr-4 py-2.5 text-sm" />
            </div>
            <div className="relative">
              ...
              <button key={p.id} onClick={() => add(p)} className="text-left rounded-lg border p-3 hover:border-primary hover:shadow-glow transition bg-card">
                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    ...
                    {p.barcode && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground"><ScanBarcode className="h-3 w-3 inline" /></span>}
                  </div>
                </div>
              </button>
            ))}
