...
const ALL: AppRole[] = ["super_admin","director","branch_manager","pharmacist","cashier","inventory_clerk"];

const NAV: NavItem[] = [
  { to: "/app/dashboard", label: "Dashboard", icon: LayoutDashboard, roles: ALL },
  { to: "/app/pos", label: "Point of Sale", icon: ShoppingCart, roles: ["super_admin","branch_manager","cashier","pharmacist"] },
  { to: "/app/inventory", label: "Inventory", icon: Package, roles: ["super_admin","director","branch_manager","pharmacist","inventory_clerk"] },
  { to: "/app/prescriptions", label: "Prescriptions", icon: FileText, roles: ["super_admin","director","branch_manager","pharmacist"] },
  { to: "/app/patients", label: "Patients", icon: Stethoscope, roles: ["super_admin","director","branch_manager","pharmacist","cashier"] },
  { to: "/app/sales", label: "Sales", icon: ShoppingCart, roles: ["super_admin","director","branch_manager","cashier"] },
  { to: "/app/financials", label: "Financials", icon: DollarSign, roles: ["super_admin","director","branch_manager"] },
    ...
    <aside className="hidden md:flex flex-col w-64 bg-sidebar text-sidebar-foreground border-r border-white/5">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        {items.map((n) => {
          ...
          return (
            <Link key={n.to} to={n.to} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition ${active ? "bg-primary text-primary-foreground shadow-glow" : "text-white/70 hover:bg-sidebar-accent hover:text-white"}`}>
      ...
      <div className="p-4 border-t border-white/5">
        ...
        <button onClick={signOut} className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-white/70 hover:bg-sidebar-accent hover:text-white">
