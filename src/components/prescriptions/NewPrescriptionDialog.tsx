...
export function NewPrescriptionDialog({ open, onOpenChange }: Props) {
  ...
  const [items, setItems] = useState<RxItem[]>([
    { product_id: "", dosage: "", frequency: "", duration: "", quantity: "" },
  ]);
  ...
  const addItem = () =>
    setItems([...items, { product_id: "", dosage: "", frequency: "", duration: "", quantity: "" }]);
  ...
  const handleSubmit = async () => {
    if (!patientId || !prescriberName) {
      toast.error("Patient and prescriber are required");
      return;
    }
    if (items.some((it) => !it.product_id || !it.quantity)) {
      toast.error("All items need a product and quantity");
      return;
    }
    setSubmitting(true);
    try {
                ...
                <div className="col-span-4">
                  ...
                  >
                    ...
                    <SelectContent>
                      {(products ?? []).map((p: any) => (
                        <SelectItem key={p.id} value={p.id}>
                          {p.name} {p.strength} ({p.dosage_form})
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
