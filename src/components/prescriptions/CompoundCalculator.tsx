...
export function CompoundCalculator({ open, onOpenChange }: Props) {
  ...
  const addIngredient = () =>
    setIngredients([...ingredients, { name: "", amountPerUnit: "", unit: "mg" }]);
      ...
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <FlaskConical className="h-5 w-5" /> Compound Medication Mixer
          </DialogTitle>
        </DialogHeader>
            ...
            {ingredients.map((ing, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 items-end mb-2">
                ...
                <div className="col-span-3">
                  <Label className="text-xs">Amount/Unit</Label>
                  <Input
                    type="number"
                    value={ing.amountPerUnit}
                    onChange={(e) => updateIngredient(i, "amountPerUnit", e.target.value)}
                    placeholder="0"
                  />
                </div>
                <div className="col-span-3">
          ...
          {validIngredients.length > 0 && targetNum > 0 && (
            <div className="rounded-lg bg-muted/30 p-4 space-y-2">
              <div className="font-display font-semibold text-sm mb-2">
                Calculation Results (for {targetQuantity} {targetUnit})
              </div>
              <table className="w-full text-sm">
                <thead className="text-muted-foreground text-xs">
                  <tr>
                    <th className="text-left pb-1">Ingredient</th>
                    <th className="text-right pb-1">Per Unit</th>
                    <th className="text-right pb-1">Total Needed</th>
                  ...
                  {calculations.map((c) => (
                    <tr key={c.name} className="border-t">
                      ...
                      <td className="py-1 text-right font-mono">
                        {c.amountPerUnit} {c.unit}
                      </td>
                      <td className="py-1 text-right font-mono font-bold">
                        {c.totalNeeded.toFixed(2)} {c.unit}
                      </td>
              ...
              <div className="flex justify-between text-sm font-bold border-t pt-2 mt-2">
                ...
                <span className="font-mono">
                  {totalWeight.toFixed(2)} {targetUnit}
                </span>
              </div>
            </div>
          )}
